...
if iscell(errBar) 
    ...
    fun2=errBar{2};
%     errBar=fun2(y);
%     P99 = tinv(0.8, size(y,2)-1);
    y2 = y;
    y2(y2 == 0) = NaN; 
    y=fun1(y2,'omitnan');
    errBar=fun2(y2,'omitnan');
%     errBar = y + fun2(y2,'omitnan')/sqrt(size(y,1))*P99;
else
    y=y(:).';
end
...
% errbarNew = errBar';
% errBar2(1,:) = smooth(errBar(1,:),30,'moving');
% errBar2(2,:) = smooth(errBar(2,:),30,'moving');
...
% Window for smoothing the median curve just for display purposes
l = 7;

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Plot to get the parameters of the line
    set(gcf,'renderer','Painters'); % New added to overcome the default renderer
    H.mainLine=plot(x,smooth(y,l,'moving'),lineProps{:},'LineWidth',6);
    ...
    uE=smooth(y,l,'moving')'+smooth(errBar(1,:),l,'moving')';
    lE=smooth(y,l,'moving')'-smooth(errBar(2,:),l,'moving')';
